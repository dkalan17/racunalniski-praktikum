...
$$5\cdot\sum_{n=1}^{\infty}\frac{n+1}{2n+3}$$
...
tip (npr. \texttt{bool} v C++), kjer bi vrednost \texttt{true} predstavljala dosegljivost, {\tt false} pa zasedenost.
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S $abc$ označimo število
kršitev omejitve $\sqrt[x]{t^{2}}$, z $g_{55\$}$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
$\iiiint\sqrt[e]{f(x)}dx$
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na $n$-tem koraku
   označimo s $e^{i}$.
